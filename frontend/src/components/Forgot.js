...
export default function Forgot(props) {
  ...
  const onSubmitHandler = async (event) => {
      ...
      try {
          ...
          const roleResponse = await axios.get(`http://localhost:4000/api/users/${email}`);
          const role = roleResponse.data.role;

          // Prepare the user object with the fetched role
          const user = {
              email: email,
          ...
          const response = await axios.put(`http://localhost:4000/api/users/${email}`, user);
